...
/**
 ...
 Examples of FRMediaTypes are image/gif, text/css and application/xhtml+xml
 */
...
/**
 Manages FRMediaTypes that are used by epubs.
 */
extension FRMediaType {
    ...
    static let FRMediaTypes = [xhtml, epub, ncx, opf, jpg, png, gif, javaScript, css, svg, ttf, ttf1, ttf2, openType, woff, mp3, mp4, ogg, smil, xpgt, pls]

    /**
     Gets the FRMediaType based on the file mimetype.
     - parameter name:     The FRMediaType name
     - parameter fileName: The file name to extract the extension
     - returns: A know FRMediaType or create a new one.
     */
    static func by(name: String, fileName: String?) -> FRMediaType {
        for FRMediaType in FRMediaTypes {
            if FRMediaType.name == name {
                return FRMediaType
            }
        }
    ...
    /**
     Gets the FRMediaType based on the file extension.
     */
    static func by(fileName: String) -> FRMediaType? {
        let ext = "." + (fileName as NSString).pathExtension
        return FRMediaTypes.filter({ $0.extensions.contains(ext) }).first
    }
     ...
     */
    static func isBitmapImage(_ FRMediaType: FRMediaType) -> Bool {
        return FRMediaType == jpg || FRMediaType == png || FRMediaType == gif
    }

    /**
     Gets the FRMediaType based on the file extension.
     */
    static func determineFRMediaType(_ fileName: String) -> FRMediaType? {
        let ext = fileName.pathExtension

        for FRMediaType in FRMediaTypes {
            if FRMediaType.defaultExtension == ext {
                return FRMediaType
            }
            if FRMediaType.extensions.contains(ext) {
                return FRMediaType
            }
        }
        return nil
